...
# Generator
# --------------------------------------------------------------------------- #
def _class_means(rng: np.random.Generator, n_classes: int, latent_dim: int) -> np.ndarray:
    """Well-separated but overlapping class centroids in latent space."""
    means = rng.normal(0, 1, size=(n_classes, latent_dim))
    # push them apart so structure is recoverable but not trivial
    means *= 2.2
    return means
...
def generate(config: Optional[GenConfig] = None):
    ...
    class_means = _class_means(rng, cfg.n_classes, cfg.latent_dim)
    ...
    for s in range(cfg.n_subjects):
        ...
        z0 = class_means[true_class] + rng.normal(0, 0.7, size=cfg.latent_dim)
